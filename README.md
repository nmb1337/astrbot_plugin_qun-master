...
- `all_members_mention_batch_size`：`/全体成员` 每条消息艾特人数（在插件设置中调整）。
...
- `/立即执行 等级`：按低等级规则立即踢出，不等待冷却时间；`kick_user_whitelist` 内成员始终不踢。
